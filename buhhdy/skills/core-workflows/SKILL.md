...
| 5 | Inception ADR promotion | **buhhdy-level** shell (`sys_os_shell`) | `promote-adr.ts <plans> --from .../architecture.md --slug <slug>` | — | — | — | IMMEDIATELY on architecture approval, before handoff (conforming repos only — skip-with-note elsewhere). See openspec-conformance "Inception promotion" |
| 6 | Epic-queue handoff | **buhhdy-level** | buhhdy | — | — | — | TERMINAL. Hands the human (or the next buhhdy run) the PRD's epic queue with "run Workflow 1 per epic" instructions. NO stories, NO tasks.md, NO tracker issues, NO index writes — each epic, when picked up, enters Workflow 1 step 1 seeded with brief/PRD/architecture, and its change's `proposal.md` links back to its PRD epic |
