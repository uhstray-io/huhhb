---
name: fanout
description: Run independent subtasks in parallel — one git worktree and one implementation sub-agent per task, each opening its own PR — then cross-review every PR. buhhdy never merges; the human does.
---
...
2. Dispatch one implementation sub-agent per task, scoped to its worktree:
   ...
   and opens its OWN PR for the branch. Do not add any co-author/attribution
...
5. buhhdy does NOT merge — the PR is the deliverable. When cross-review passes,
