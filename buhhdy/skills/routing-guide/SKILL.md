...
| calibration-refresh         | buhhdy-level | —            | Provider/model/auth fact maintenance (claims ledger → cheapest-first verification → in-place updates to the config/MODEL-MANIFEST calibration defaults, one PR per run) — buhhdy runs it itself; only verification probes are dispatched (one LIGHTWEIGHT per provider; gemini-standard/codex STANDARD web checks, cross-vendor for consequential claims). Never changes routing autonomously |
