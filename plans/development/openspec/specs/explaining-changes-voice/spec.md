# explaining-changes-voice Specification

## Purpose
Governs the voice of change narration — what an explanation must name, the
specificity it owes the reader, and the vagueness it may not fall back on — so
that narration educates rather than reporting an edit log.

> **Partially as-built.** The skill content shipped in PR #50, but its change
> (`voice-anatomy-layer`) reads 0/40 tasks — the work was never verified
> task-by-task, so treat these requirements as implemented-but-unconfirmed.
