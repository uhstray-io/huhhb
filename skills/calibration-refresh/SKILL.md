...
| 2a | **Verify: availability + auth** | Dispatched | one LIGHTWEIGHT dispatch per provider (gemini-lite for gemini) | `sys_list_models` plus one live round-trip per provider — the round-trip doubles as the auth check. A failed ID here is a finding, not an error |
| 2b | **Verify: deprecations / pricing / releases** | Dispatched | gemini-standard or codex (STANDARD); BOTH for consequential claims | Web verification against provider primary docs. A consequential claim (anything a routing rule, tier slot, or cost basis rests on) flips ONLY on cross-vendor confirmation: two independent sources, or two different-vendor agents each citing primary docs. One source is a lead, not a flip |
| 2c | **Verify: CLI / harness behaviors** | buhhdy-level shell | — | Check against what is actually installed on this machine: CLI `--version`/`--help`, `opencode models`, the installed omnigent runtime source. Never verify a harness claim from docs alone when the binary is right there |
| 3 | **Record** | buhhdy-level | — | Write a fresh `kind: calibration` record (per the Record Contract) for every claim that CHANGED or is newly discovered; supersede contradicted records via the metadata-only status flip. Claims verified UNCHANGED are recorded in the ledger and report only — never as sweep-marker records; freshness lives in the report, and the store stays compact. Contradictions BETWEEN sources are never resolved silently: record both sources verbatim, mark the claim `disputed`, and escalate (house style: config's gemini-2.5-flash-lite disputed-retirement note — state both positions, name both sources, pick the migration-safe action only if one exists) |
...
- **No interactive prerequisites.** If the run stalls on a question that
  ...
  violated.
- Records pass the Record Contract write lint: observational only, no
  imperatives, no routing/permission references.
