...
| `merge` | overlapping descriptions (lint S5 near-misses) or the same task class split across skills | judge on four axes (R2): job-to-be-done, deliverable type, hard constraints, tools/workflow — merge only when the same capability remains after removing instance details; **an unsafe merge degrades to add**, and low-confidence judgment falls back to the structural overlap score. One general variant absorbs; others → archive proposal |
