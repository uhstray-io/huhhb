...
|---|------|-------|--------------|
| M1 | MemPalace | `command -v mempalace`; if present, `mempalace_status` succeeds; nexus lives at its DEFAULT path | Absent = **warn** (enhancement, not dependency) with install/init steps from the `memory` skill. A nexus inside ANY repo path = **fail** — path-separation violation |
| M2 | evolve / Honcho | `node scripts/evolve/honcho_client.ts status` (huhhb checkout) — classify: configured (team endpoint) / local mode / unconfigured. Verify the resolved `user__<profile-id>` is present and NOT a placeholder or shared/team id — each teammate accrues to their own peer | Unconfigured = **warn** with terminal-only guidance: the user runs `honcho_client.ts init` THEMSELVES in a terminal. Placeholder/shared profile id = **fail** |
...
| # | Item | Check | Status rules |
|---|------|-------|--------------|
| P1 | repo-memory present | `.claude/memory/` + `MEMORY.md` exist. If missing: run the `repo-memory` skill's own First Run setup — the ONE auto-fix this skill may apply unasked (safe, idempotent). Verify AGENTS.md carries the `## Repo Memory` heading | Missing = auto-initialize, then **pass**; block present but targeting CLAUDE.md = **warn** with the one-line fix (move it to AGENTS.md; CLAUDE.md stays a pointer) |
