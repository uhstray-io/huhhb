...
description: Use when a large function needs decomposing into named single-purpose units — extract computation blocks, guard clauses, or setup phases as separate functions, or move a function to a better module. Triggers on "strict refactor", "/strict-refactor", "extract function", "break up this function", "split into smaller functions". Changes structure only — no logic, no renames, no new abstractions.
---
...
restructure control flow, or introduce abstractions. Structural decomposition only.
...
4. **One logical result** — the block produces a value, a tuple of tightly related values,
   or pure side effects — not a mix of unrelated mutations.
...
> any one check is not a candidate. If you cannot write a clean one-line "why equivalent"
> justification, it is not a candidate.

A single call site is not a reason to skip. A block with a meaningful boundary and an
obvious name is justified by readability alone.
...
These name common shapes; they are **not a closed list.** Any mechanically extractable
block with a clear name and single responsibility is a candidate. Ambition applies to
*discovery* — the extraction gate is non-negotiable.
...
- No introducing abstractions, interfaces, or base classes.
